...
content_hash: d385c16490ce2c719c0b503dbb23696017b761276b0e2fdd293e4a6e4adc5b2f
---
...
2. **If `worktree.enabled` is true → call the worktree CLI.**
   ...
   ```
   uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.4 python -m harness_maker.worktree create execute "$(pwd)"
   ```
...
4. **Finalize through the CLI, not direct merge/cleanup calls.**
   ...
   user-facing commit:
   ```bash
   uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.4 python -m harness_maker.worktree finalize <WT> stage-only
   ```

   Failure preserves the worktree for inspection:
   ```bash
   uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.4 python -m harness_maker.worktree finalize <WT> fail
   ```
   ...
   ```bash
   HM_OWNED_SESSION_UUIDS="$(uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.4 python -m harness_maker.worktree owned-crumb-read "$(pwd)" <slug>)" uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.4 python -m harness_maker.worktree post-commit-pop "$(pwd)"
   ```

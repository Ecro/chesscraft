...
content_hash: e416e389bbc07b69cf8f79ddaecdf1f990fd0d9ba17f5aa840f936ea8ee75dc7
---
...
2. **If `worktree.enabled` is true → call the worktree CLI.**
   ...
   ```
   uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 python -m harness_maker.worktree create execute "$(pwd)"
   ```
...
4. **Finalize through the CLI, not direct merge/cleanup calls.**
   ...
   user-facing commit:
   ```bash
   uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 python -m harness_maker.worktree finalize <WT> stage-only
   ```

   Failure preserves the worktree for inspection:
   ```bash
   uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 python -m harness_maker.worktree finalize <WT> fail
   ```
   ...
   ```bash
   HM_OWNED_SESSION_UUIDS="$(uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 python -m harness_maker.worktree owned-crumb-read "$(pwd)" <slug>)" uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 python -m harness_maker.worktree post-commit-pop "$(pwd)"
   ```

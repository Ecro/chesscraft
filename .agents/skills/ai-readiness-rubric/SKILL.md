...
content_hash: 3cf0ba0c9b171771c5c75c22a08c4b874acb0b108597e100e59c812584bd64d3
---
...
| 3 cache | `harness_maker.cache_diagnostics` | Claude Code **session transcript** analysis (`~/.claude/projects/**`) via `diagnose_cache_from_turns`. Classifies miss reasons: `min_threshold` (prefix below the model's published cache-write minimum), `unknown_model` (no published minimum on record — that test could not run, so the diagnosis is incomplete), `invalidation` (prefix mutated), `ttl` (gap exceeded the TTL tier that applied — 5 min or 1 h, derived per turn), `first` (expected). |

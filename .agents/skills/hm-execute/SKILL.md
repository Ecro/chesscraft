...
content_hash: adc07cb329b3525087a0c4171d3ac4a7287a8555e3e2ca54b6eee3653bb7f5f9
---
...
Invoke via `@hm-execute <slug> [no-tdd]`.

Parse from the user's natural-language input:
- First token or "slug: <name>" → the task slug (required)
- "no-tdd" keyword → set `tdd_active = false`
...
```
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm worktree task-preflight <slug> \"$(pwd)\" --stage hm:execute --claude-session-id \"$HM_SESSION_ID\"")
```
...
Parse from the user's natural-language input:
- First token or "slug: <name>" → task slug
- "no-tdd" keyword → set `tdd_active = false`
...
**Record every round (ADR-004).** One row per **round**, `--pass` being the round number. The
rule predates the single-dispatch collapse and survives it: it is keyed on the round, not on the
dispatch count, so a restored fan-out would not collide on `(agent, stage, run-id, pass)` or
leave `--terminal` unowned.
...
```
Bash("cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm stage_agent_ledger emit --run-id '<run-id>' --agent test-reviewer --stage execute --slug '{slug}' --pass <round-number> --verdict '<PASS|FAIL>' --terminal --duration-ms '<elapsed>' --barrier-index '<segment>'")
```
...
```
Bash("cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm test_dep_map --root . --changed-file <f1> …")
...
```
Bash("cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm spec_mutation gate --yaml specs/SPEC-{slug}.machine.yaml --tier 1")
```
...
```
Bash("if [ -f \"<WT>/.claude/.hm-iter-receipts/.current-iter\" ]; then ITER=$(cat \"<WT>/.claude/.hm-iter-receipts/.current-iter\" 2>/dev/null); if [ -n \"$ITER\" ]; then uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm iter_receipts write --iter \"$ITER\" --stage execute --verdict <verdict> --root \"<WT>\"; fi; fi")
```
...
You **MAY** call `request_user_input` (autoloop exception) **ONLY IF** `[finalize] stash-pop conflict` OR `[finalize] untracked-file collision` appears in finalize's stderr. Any other failure: halt with the stderr message, do NOT ask.
...
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm worktree finalize <WT> stage-only")
```

```
# Stage halted on a blocker — preserve worktree for inspection:
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm worktree finalize <WT> fail")
```
...
```
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm worktree owned-crumb-add \"$(pwd)\" <slug> \"$(uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm worktree wt-uuid <WT>)\"")
```
...
```
Bash("HM_OWNED_SESSION_UUIDS=\"$(uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm worktree owned-crumb-read \"$(pwd)\" <slug>)\" uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm worktree post-commit-pop \"$(pwd)\"")
```
...
<!-- @hm:user:extensions -->
<!-- Project-specific additions to the hm-execute skill. Preserved across upgrades. -->
<!-- @hm:/user:extensions -->

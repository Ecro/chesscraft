...
content_hash: 9a91a67472eee39ee590a3e45cb74bad812a573797f12f54321c7b4b1bcbc0fe
---
...
Invoke via `@hm-research <topic> [deep] [slug: <name>]`.

Parse from the user's natural-language input:
- Main topic text → the research topic
- "deep" keyword → enable `--deep` refinement interview
- "slug: <name>" → explicit output slug
...
- User topic (from your message — parse the research topic from natural language).
...
```bash
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.5 hm second_brain search '<topic terms>' --type reference")
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.5 hm second_brain search '<topic terms>' --type project")
```
...
```
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.5 hm worktree task-preflight <slug> \"$(pwd)\" --stage hm:research --claude-session-id \"$HM_SESSION_ID\"")
```
...
When `--deep` is set, use the `request_user_input` tool (if unavailable, ask in your response) with 3-5 questions drawn from this rubric. Use the configured locale for the live interview: the round preamble, ambiguity explanation, question text and option labels, and any validation prompt must be in `ko` (en→English, ko→Korean, ja→Japanese, others→English fallback). The persisted RESEARCH document remains English unless project policy says otherwise.
...
Write to `<WT>/work-docs/RESEARCH-{slug}.md`. `/hm:plan` Step 2 reads this file via PLAN frontmatter `research_doc:` and skips its own retrieval — this is the single biggest token saver in the workflow.
...
```
Bash("if [ -f \"<WT>/.claude/.hm-iter-receipts/.current-iter\" ]; then ITER=$(cat \"<WT>/.claude/.hm-iter-receipts/.current-iter\" 2>/dev/null); if [ -n \"$ITER\" ]; then uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.5 hm iter_receipts write --iter \"$ITER\" --stage research --verdict <verdict> --root \"<WT>\"; fi; fi")
```
...
<!-- @hm:user:extensions -->
<!-- Project-specific additions to the hm-research skill. Preserved across upgrades. -->
<!-- @hm:/user:extensions -->

...
content_hash: 742e998a744d58e6cebac80fb5c904c21acac078352a50fde83e87812bd10a38
---
...
```
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.55.0 hm worktree create execute \"$(pwd)\" --claude-session-id \"$HM_SESSION_ID\"")
```
...
```
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.55.0 hm worktree verify <WT>")
```
...
3.5. **Write `.current-iter` marker** (Gate 0 contract — ADR-001 of PLAN-loop-mid-stop-and-review-skip).
   ...
   ```
   Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.55.0 hm iter_receipts set-iter-marker --iter <N> --root \"<WT>\"")
   ```
...
4.5. **Gate 0 — Receipt verification** (ADR-001 + ADR-005 of PLAN-loop-mid-stop-and-review-skip).
   ...
   ```
   Bash("cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.55.0 hm iter_receipts verify --iter <N> --expected <EXPECTED_STAGES> --root \"<WT>\"")
   ```
   ...
   **Auto-retry semantics (ADR-005, cap=2 per (iter, stage)):**
   ...
   2. Read the current value of `stage_retry_counts[K]` from
      ...
      ```
      Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.55.0 hm iter_receipts patch-runtime --context work-docs/loop-context/<slug>.yaml --counter stage_retry_counts --key iter-<N>:<stage> --value <count>")
      ```
   ...
   4. If `stage_retry_counts[K] >= 3`:
      ...
      - **B. Skip with explicit `verdict: skipped` marker** — invoke the
        ...
        `uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.55.0 hm iter_receipts write --iter <N> --stage <stage> --verdict skipped --root "<WT>"`
...
0. **Clear `stage_retry_counts`** (Gate 0 hygiene — ADR-005 of PLAN-loop-mid-stop-and-review-skip).
   ...
   ```
   Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.55.0 hm iter_receipts patch-runtime --context work-docs/loop-context/<slug>.yaml --counter stage_retry_counts --clear")
   ```
...
5. **Finalize worktree** (only if engaged in step 5):
   ...
   ```
   Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.55.0 hm worktree finalize <WT> <STATUS>")
   ```

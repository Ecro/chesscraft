...
content_hash: 1253c0605e8274611b6916a39fe7caf5fae66b7344341a709e09249b51eaaa0d
---
...
```bash
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.5 hm memory_retrieve --topic '<topic>' --k 6 --pre-k 30")
```
...
```bash
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.5 hm second_brain search '<task slug or topic>' --type decision")
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.5 hm second_brain search '<task slug or topic>' --type preference")
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.5 hm second_brain search '<task slug or topic>' --type project")
```
...
```
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.5 hm worktree task-preflight <slug> \"$(pwd)\" --stage hm:plan --claude-session-id \"$HM_SESSION_ID\"")
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.5 hm worktree loop-mode-active "<PROJECT_ROOT>" --claude-session-id "$HM_SESSION_ID"
```
...
```bash
uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.5 hm second_opinion_invoke --model codex --prompt-file <the literal path printed above> --slug "<slug>" --stage plan
```
...
```bash
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.5 hm plan_rounds plan --file <the literal temp path> <--previous PASS-1's path, from pass 2 on> <--churn-ratio R when Step 4.4 measured it>")
```
...
```bash
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.5 hm plan_rounds outcome --file <pass 2's critiques> --previous <pass 1's critiques>")
```
...
```
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.5 hm stage_agent_ledger emit --run-id '<run-id>' --agent plan-validator --stage plan --slug '{slug}' --pass <N> --verdict '<APPROVED|NEEDS_REVISION|MAJOR_REVISION>' --terminal --duration-ms '<elapsed>' --barrier-index '<segment>'")
```
...
```
Bash("if [ -f \"<WT>/.claude/.hm-iter-receipts/.current-iter\" ]; then ITER=$(cat \"<WT>/.claude/.hm-iter-receipts/.current-iter\" 2>/dev/null); if [ -n \"$ITER\" ]; then uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.5 hm iter_receipts write --iter \"$ITER\" --stage plan --verdict <verdict> --root \"<WT>\"; fi; fi")
```

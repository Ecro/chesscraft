...
content_hash: bce9765c5ef7c8511d29a1309150b022a54d67b8bf8eb9beef03f16a6cb7f42f
---
...
```
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm worktree task-preflight <slug> \"$(pwd)\" --stage hm:wrapup --claude-session-id \"$HM_SESSION_ID\"")
```
...
```
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm observability.verification_cache check --root . --mode relevant")
```
...
Run the project's full check suite only when the marker is absent or stale, and take the
commands from the project's CI rather than an example — an example that is narrower than CI
passes locally and fails on push, saying nothing about what it skipped:


```
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm verification_plan commands --root .")
```


Run each printed command verbatim. **Exit 1 = degraded** (no CI, unreadable, or nothing
recognised — stderr says which): only then fall back to the project's toolchain
(`uv run pytest -x` / `ruff check .` / `ruff format --check .` / `mypy --strict`), say the
gates were guessed. Never drop the format check: lint alone misses format violations
(`[fail:lint] ruff-format-not-in-local-verify-pass` count:2).
...
```
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm observability.verification_cache mark-pass --root . --mode relevant --checks lint,format,mypy,pytest")
```
...
```
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm spec_machine mark-tested --yaml specs/SPEC-{slug}.machine.yaml --md specs/SPEC-{slug}.md --test-id AC-001=tests/path::test_name")
```
...
```
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm spec_machine mark-judged --yaml specs/SPEC-{slug}.machine.yaml --ac AC-NNN --verdict <reviewer's pass|fail> --evidence-file <reviewer evidence_summary file> --root .")
```
...
```
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm spec_machine waiver-check --yaml specs/SPEC-{slug}.machine.yaml --dev-mode task-driven --root .")
```
...
```
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm memory_md upsert-wiki --root . --slug '<slug>' --category '<category>' --body-file <tmpfile>")
```
...
```
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm memory_retrieve --topic \"<symptom / root cause>\" --k 6 --pre-k 30")
```
...
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm memory_md upsert-failure --root . --slug '<slug>' --category '<category>' --body-file <tmpfile>")
# Recurrence (confident same-root-cause match) — reuse the EXACT slug + one-line note:
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm memory_md upsert-failure --root . --slug '<existing-slug>' --category '<category>' --occurrence-note '<one line: what happened this time>'")
```
...
```
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm second_brain promote --type <decision|failure|preference|project|reference|journal> --source-slug '<stable-local-slug>' --title '<title>' --body-file <path>")
```
...
Print `[intent] not in use` and skip this step when `.claude/intent.yaml` is absent or `hm world status --json` reports `state: not_filled_in`. Otherwise, in order — and **never write without the answer**:

<!-- @hm:answer-gated:assumption -->
Ask with `request_user_input`: **"Did this task confirm, supersede or contradict a recorded assumption?"** — options: one per assumption id (from the status output), plus **"no"**.
If the answer is "yes": (an assumption was named) run ONCE, with the id, relation and text the operator confirmed:
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm world assume observe <id> --relation <confirms|supersedes|contradicts> --text '<what was observed>' --observed-at <ISO-8601 UTC> [--claim '<new claim>' — required when the relation is supersedes]")
Otherwise: write nothing.
<!-- /@hm:answer-gated -->

<!-- @hm:answer-gated:objective-close -->
Only when the PLAN frontmatter carries `objective: <id>`. Ask with `request_user_input`: **"Close objective <id> now?"** — options: **"yes — met"**, **"yes — missed"**, **"yes — no_data"**, **"no, keep it open"**.
If the answer is "yes": run ONCE, with the observed value chosen and a one-line note from the operator:
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm world objective close <id> --observed <met|missed|no_data> --note '<operator note>'")
Otherwise: write nothing.
<!-- /@hm:answer-gated -->

<!-- @hm:answer-gated:outcome-measure -->
Only when `hm world gap --json` lists an outcome with `measure: true`. Ask with `request_user_input`: **"Measure outcomes now?"** — list each measurable outcome with its `last` value and `observed_at` age from that output; options: **"yes — measure all"**, **"no"**.
If the answer is "yes": run ONCE (it records every measurable outcome that succeeds and reports the rest; a non-zero exit means one failed, never that nothing was written):
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm world outcome measure --all")
Otherwise: write nothing.
<!-- /@hm:answer-gated -->

### Steps 6 → 7.6 — Stage, commit, pop, drain (ONE call)
> **Flag-on (per-task worktree):** staging + commit MUST land inside `<WT>` — the
> `hm/<slug>` task worktree from the Step-0 preflight — so they reach the task branch
> that Step 7.7 squash-lands. In the base repo the index is empty → the commit is a
> no-op and the curated message never reaches the branch (which also defeats Step 7.7's
> reuse of it). `--worktree` and `--base` are separate explicit arguments below, and the
> call **refuses** a `--worktree` that is not a worktree of `--base`, so the cwd of the
> shell that runs it cannot silently retarget the commit.
First write the commit message to a file. `<type>(<scope>): <subject ≤72 chars>` on line
...
Step 7.7 below is deliberately NOT in it: it is the only step that can lose work, so it
keeps its own invocation and its own operator decision point (ADR-006).
**First, produce the ledger roll-up** — the manifest stages it; nothing else writes it. Rows come
from the **base** repo; the document lands where this wrapup commits. On `invalid choice:
'rollup'` the pinned plugin predates the subcommand: note it, continue (the file is optional),
never hand-write it:


```
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm autopilot_ledger rollup --root <WT> --write")
```



```
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm wrapup_land --worktree <WT> --base <BASE> --slug <slug> --message-file <msg-tmpfile> --required work-docs/PLAN-{slug}.md --optional .claude/memory/ --optional work-docs/REVIEW-{slug}-*.md --optional work-docs/RESEARCH-{slug}.md --optional specs/SPEC-{slug}.md --optional specs/SPEC-{slug}.machine.yaml --optional work-docs/BASELINE-ledger-rollup.md")
```
...
You **MAY** call `request_user_input` (autoloop exception) **ONLY IF** the literal substring `[finalize] stash-pop conflict` OR `[finalize] untracked-file collision` appears in stderr. Any other non-zero exit: surface verbatim and halt, do NOT ask.
...
```
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm worktree task-land <SLUG> <BASE>")
```
...
```
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm worktree commit-base-memory <BASE> --expect-head <SQUASH_SHA>")
```
...
```
Bash("if [ -f \"<WT>/.claude/.hm-iter-receipts/.current-iter\" ]; then ITER=$(cat \"<WT>/.claude/.hm-iter-receipts/.current-iter\" 2>/dev/null); if [ -n \"$ITER\" ]; then uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm iter_receipts write --iter \"$ITER\" --stage wrapup --verdict <verdict> --root \"<WT>\"; fi; fi")
```

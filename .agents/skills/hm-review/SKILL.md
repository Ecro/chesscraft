...
content_hash: b14eaf578e30eef1eefe847ce797c83f4bff23597e73baeb9c091342fcc4504f
---
...
```bash
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 hm second_brain search '<changed area or task slug>' --type failure")
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 hm second_brain search '<changed area or task slug>' --type preference")
```
...
```
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 hm worktree task-preflight <slug> \"$(pwd)\" --stage hm:review --claude-session-id \"$HM_SESSION_ID\"")
```
...
```
Bash("cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 hm review_run open --slug <slug>")
```
...
- Resolve `review_base` once, here in round 1, storing it at `refs/hm-freeze/v1/<slug>-base`:
  ...
  ```bash
  !cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 hm freeze resolve-base --slug <slug>
  ```
...
```bash
Bash("cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 hm lens_coverage check --results-dir .claude/observability/.hm-lens-results --slug <slug> --round <round> --run-id <run-id> --preset Side")
```
...
4. Merge the two passes via the harness CLI. `Write` `{"pass1": [...], "pass2": [...]}`
   ...
   ```bash
   Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 hm two_pass_review merge --file <the literal temp path>")
   ```
...
```bash
!cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 hm codex_adapter stamp-ids < <the literal temp path>
```
...
```
Bash("cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 hm stage_agent_ledger persist-payload --file <the literal temp path> --slug {slug} --run-id <run-id> --round <N> --reviewer merged")
```
...
```bash
uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 hm second_opinion_invoke --model codex --prompt-file <the literal path printed above> --slug "<slug>" --stage review
```
...
```bash
Bash("cd <WT> && set -o pipefail; uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 hm review_consensus finalize --file <the literal temp path> --slug {slug} --round <N> | tee <FINALIZE-r{N}>")
```
...
```bash
!cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 hm second_opinion_invoke --record-disposition --disposition-file <the literal temp path> --slug <slug> --stage review
```
...
```bash
Bash("cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 hm lens_coverage check --results-dir .claude/observability/.hm-lens-results --slug <slug> --round 1 --round 2 <one --round per further round so far> --round <this round> --run-id <run-id> --preset Side")
```
...
```bash
Bash("cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 hm review_churn pin --slug {slug} --label r{N}-pre")
```
...
```bash
Bash("cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 hm review_consensus plan --churn-ratio <the measured ratio> --threshold 0.2")
```
...
```bash
!cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 hm freeze commit --slug <slug> --pass <confirm-1|confirm-2>
```
...
```bash
!cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 hm freeze read-base --slug <slug>
```
...
```bash
Bash("cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 hm lens_coverage check --results-dir .claude/observability/.hm-lens-results --slug <slug> --round <pass-id> --run-id <run-id> --preset Side")
```
...
```bash
!cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 hm freeze reap --slug <slug>
```
...
```
Bash("cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 hm stage_agent_ledger emit --run-id '<run-id>' --agent confirmation-pass --stage review --slug '{slug}' --pass <1|2> --verdict '<PASS|FAIL>' --duration-ms '<elapsed>'")
```
...
```bash
Bash("cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 hm review_churn oscillation --slug {slug} --rounds <rounds>")
```
...
```bash
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 hm review_telemetry emit --file <the literal temp path> --measured <FINALIZE-r{N}> <plus --measured <CHURN-r{N}> from round 2>")
```
...
```
Bash("if [ -f \"<WT>/.claude/.hm-iter-receipts/.current-iter\" ]; then ITER=$(cat \"<WT>/.claude/.hm-iter-receipts/.current-iter\" 2>/dev/null); if [ -n \"$ITER\" ]; then uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 hm iter_receipts write --iter \"$ITER\" --stage review --verdict <verdict> --root \"<WT>\"; fi; fi")
```

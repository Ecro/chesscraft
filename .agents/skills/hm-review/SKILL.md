...
content_hash: dfcfee4d9a5847c923c965e23233f4690473d23f9fc579fb7d2b1b86e2a7e9fa
---
...
```bash
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm second_brain search '<changed area or task slug>' --type failure")
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm second_brain search '<changed area or task slug>' --type preference")
```
...
```
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm worktree task-preflight <slug> \"$(pwd)\" --stage hm:review --claude-session-id \"$HM_SESSION_ID\"")
```
...
Every cap in this stage — `max_review_rounds`, the no-progress invariant, the two-pass
confirmation bound — is state inside ONE run. Nothing used to record that a run was still open,
so a second `/hm:review` started a parallel one and reset all of them. On 2026-08-17 a single
session created six runs for one slug that way.


```
Bash("cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm review_run open --slug <slug>")
```


**Read it from `open`, never mint one:** the `id` this prints is the run identity every
`<run-id>` below stands for — the join key for the lens-results directory and every check or
record keyed on it.

**A non-zero exit means the slug already has an open run**, and the payload names it. **Resume
that run** — do not start another. Recovery from a genuinely abandoned run is
`open --force`, which reports the run it displaced; there is no expiry, because a long review
and an abandoned one are indistinguishable from elapsed time alone.

### Step 1 — Reviewer set selection

**The discovery axis** — `design`, `functionality`, `robustness`, `consistency` —
is exercised every round. These are review categories, not agent names: the core ones share an agent
AND **one dispatch** carrying all their questions. Distinct categories find distinct defects: that is
why one lens's finding stands on its own (Step 4). This list is what must be *exercised*; Step 3
says how many dispatches the round sends.
...
- Resolve `review_base` once, here in round 1, storing it at `refs/hm-freeze/v1/<slug>-base`:
  ...
  ```bash
  !cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm freeze resolve-base --slug <slug>
  ```
...
interview when slots were skipped as common-ground), cross-reference
...
**Round 1 — dispatch the 7 lenses as 4 dispatches, in ONE
message.** The core lenses share `code-reviewer` and go in one call carrying all their questions;
each domain lens keeps a call of its own. Send every lens line verbatim.
...
```
spawn_agent(agent_type="code-reviewer", message="<brief>\n\nYou are ACCOUNTABLE for all 4 lenses below; tag each finding with the lens it came from.\n\n- design — boundaries, coupling, whether this is the right shape for the problem; and complexity: could it be simpler? Unnecessary indirection, dead generality, a knob or a function with no caller on any path a user reaches.\n- functionality — does it do what the SPEC and the invariants say, on every path?\n- robustness — edge cases, partial writes, restart, resource exhaustion, recovery.\n- consistency — do the names, docstrings and declarations say what the code actually does, and does this match the conventions around it? A name or a docstring that makes a reader believe something FALSE about behaviour is a defect, not a nit; so is a second source of truth for something that already had one.")
...
its findings JSON to the per-round results directory, adding the `"run_id"` Step 0's `open` printed
and a **file-level** `"lens"` key naming the **group** — the `<group>.json` stem below. **Every
finding inside carries its own `"lens"`: the member lens that raised it**, one of `design` `functionality` `robustness` `consistency` — never the
group name. Same value at both levels for a single-lens group; different for a merged one.
...
```
.claude/observability/.hm-lens-results/<slug>/<run-id>/<round>/core.json
...
```bash
Bash("cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm lens_coverage check --results-dir .claude/observability/.hm-lens-results --slug <slug> --round <round> --run-id <run-id> --preset Side")
```
...
4. Merge the two passes via the harness CLI. `Write` `{"pass1": [...], "pass2": [...]}`
   ...
   ```bash
   Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm two_pass_review merge --file <the literal temp path>")
   ```
...
Runs only when `work-docs/PLAN-{slug}.md`'s frontmatter carries `objective: <id>`; otherwise print `[intent] no objective link — skipping` and continue. `<id>` must match `[A-Z0-9-]+`; on mismatch print `[intent] objective id malformed — skipping` and do not run the command (frontmatter text never reaches the shell unchecked). Load the record:

```bash
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm world objective show <id> --json")
```

Judge the PLAN's Implementation Plan against the record's `scope`, `non_scope` and `hypothesis`, and add one finding per drift to the merged list **before Step 3.4 stamps ids** — severity **P2**, `source: "main-loop"`, category `scope_drift`, `file` = the PLAN path, `message` opening with the objective id (`<id>: …`) so the finding names which record it was judged against:
- work added outside `scope` (whether or not `non_scope` names it) → cite the offending PLAN item;
- work added inside `non_scope` → cite the item;
- the subject of `hypothesis` omitted → cite the requirement and where in the PLAN it is missing.
No halt, no auto-edit of the record, no gate: P2 keeps it out of `unverified_severe` by construction (SPEC S10).
...
```bash
!cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm codex_adapter stamp-ids < <the literal temp path>
```
...
```
Bash("cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm stage_agent_ledger persist-payload --file <the literal temp path> --slug {slug} --run-id <run-id> --round <N> --reviewer merged")
```


> **`<run-id>` is the id Step 0's `open` printed** — read it from `open`, and keep it
...
> **⏱️ Run the invoker in the BACKGROUND, so it overlaps Pass 1.** Every invoker call below is a
> Bash tool call — give it **`run_in_background: true`**, then dispatch the Pass 1 reviewer fan-out
> **without waiting**. Collect the invoker's JSON line when you need it, at the Step 4 filter.
>
> This is the mechanism the concurrency decision always assumed and never had. The instruction to
> run the cross-model call "concurrently with Pass 1" shipped as prose with nothing behind it —
> `run_in_background` appeared **nowhere** in this harness — so the foreground Bash blocked for up
> to `CODEX_TIMEOUT_S=300` before the first reviewer was even dispatched. Nothing in the cross-model
> path consumes Pass 1's output, which is why the overlap is safe rather than merely faster.
>
> **A render-grep for `run_in_background` is a precondition, never the proof.** Whether the call
> actually overlaps is a property of one real dispatch; fixture-shaped output proves the validator
...
```bash
uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm second_opinion_invoke --model codex --prompt-file <the literal path printed above> --slug "<slug>" --stage review
```
...
```bash
Bash("cd <WT> && set -o pipefail; uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm review_consensus finalize --file <the literal temp path> --slug {slug} --round <N> | tee <FINALIZE-r{N}>")
```

It **reads only** and prints one payload: `{"findings": [...with `tag`...], "grade": …,
"counts": …, "disposition_counts": …, "human_review_needed": …, "errors": [...]}`. **Use its answer**; do not re-derive
...
| `unresolved` | The fixer read the covering test and refused (Step 4) | `oracle-blocked` |
...
```bash
!cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm second_opinion_invoke --record-disposition --disposition-file <the literal temp path> --slug <slug> --stage review
```
...
IF auto_fix disabled (config OR --no-auto-fix):
  ...
  → Close the run: `hm review_run close --slug <slug> --run-id <run-id> --outcome CHANGES_REQUESTED`.
...
Re-dispatch what the previous check named in `missing` — read that list from the CLI's output,
never from your own recollection of which dispatch failed. `missing` names **member lenses** while
the dispatch unit is a **group**, so a missing member re-dispatches **its whole group** and writes
**that group's file** — read both off the dispatch list above, never from a worked example: the
grouping is one constant, and an example naming today's is wrong the moment it changes. Write the
results into this round's directory.
...
```bash
Bash("cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm lens_coverage check --results-dir .claude/observability/.hm-lens-results --slug <slug> --round 1 --round 2 <one --round per further round so far> --round <this round> --run-id <run-id> --preset Side")
```
...
3. **Select fixable findings** — only:
   - Severity **P0 or P1**. P2 and P3 only when the current grade is D or F.
   - Tag = `consensus-passed`.
   - Has a concrete `suggestion` with replacement code (skip vague advice).

   > **P2 was in this queue and bought nothing the gate can read.** P2/P3 cannot move the
   > grade, so every P2 fixed was churn against a blind gate — and an adjudicated set of this
   > shape ran 31% real against 69% naming, comments and structure: accurate, mostly not worth
   > a round trip. The `consistency` lens is what made that volume reachable. They are **not
   > dropped** — Step 4e dispositions every finding on every path, so a skipped P2 sits in the
   > report for a human sweep. D/F keeps the old breadth, where it earns the round trip.

3b. **Pin the pre-fix endpoint** — before this round's first `Edit`, never after:

```bash
Bash("cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm review_churn pin --slug {slug} --label r{N}-pre")
```
...
<!-- @hm:oracle-blocked -->
   > **Read the covering test before this file's first `Edit`, and be willing to refuse.**
   > `targeted-test-selection` §6 owns the rule and the three outcomes; do not restate it here.
   > A refusal is recorded twice: retag the finding **`manual-only`**, and set
   > `disposition: unresolved` / `authority: oracle-blocked`. The retag is what excludes it from
   > the next round's selection above and from the grade — no second filter exists. "No covering
   > test found" is a refusal, not permission.
<!-- @hm:/oracle-blocked -->
...
5. **Verify build** — follow the `targeted-test-selection` skill, whatever the language.
   ...
   > **Classify the failure first — `targeted-test-selection` §4.5 owns the three-way split**
   > (production reaches the state / it does not / the target is too narrow for the fix). Two
   > things are review's own. **Revert either way**: the classification changes the record, not
   > the action. And the record is per-cause, never one line for all three —
   > `reverted — build/link error: <missing symbol or unit>` when nothing compiled or linked,
   > `reverted — test pins unreachable state: <upstream fact>` for the unreachable case, and
   > `caused build failure` **only** for a fix that built and failed an assertion. Collapsing
   > them records a scope problem as a bad fix, and the next round re-derives the finding
   > because nothing said the fix itself was sound.

   > On §4.5's unreachable case, carry one `manual-only` P1 `spec_gap`, as an oscillation is
   > carried — never grade-lowering, never authority to edit the test. Measured, and since
   > superseded: four runs broke the same four tests and the finding driving those fixes was
   > adjudicated a false positive, but re-running the loop with the reviewer able to read the
   > repository produced those breaks **0 times in 10 rounds**. Ask the question anyway — the
   > usual cause is a reviewer that could not see the caller, not a test that over-specifies.

5b. **Measure this round's churn** — after Step 5's reverts, so a reverted fix does not count:

```bash
Bash("cd <WT> && set -o pipefail; uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm review_churn pin --slug {slug} --label r{N}-post && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm review_churn measure --slug {slug} --round {N} --pre refs/hm-churn/v1/{slug}-r{N}-pre --post refs/hm-churn/v1/{slug}-r{N}-post | tee <CHURN-r{N}>")
```



   `tee` keeps this payload for the Telemetry Emit step, which reads the four `churn_*` keys
   out of it. Carry them into the **iteration record** only, never into the row.
...
```bash
Bash("cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm review_consensus plan --churn-ratio <the measured ratio> --threshold 0.2")
```
...
7b. **Evaluate the progress invariant.** No lifecycle transition this round → stop now with
   `CHANGES_REQUESTED` and exit reason `no-progress`. No-progress is stage-terminal, so close the
   run: `hm review_run close --slug <slug> --run-id <run-id> --outcome CHANGES_REQUESTED`, **attaching Step 5b's `churn_ratio`** (null when nothing was measurable — record the null, do not omit the field). A stall that churned a lot and a stall that churned nothing are opposite diagnoses; the exit reason alone cannot tell them apart, which is why the gate's threshold is recalibrated from these rows.
...
```bash
!cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm freeze commit --slug <slug> --pass <confirm-1|confirm-2>
```
...
```bash
!cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm freeze read-base --slug <slug>
```
...
### Step C2 — Dispatch all 7 lenses as 4 dispatches over `review_base..<freeze commit>`
...
```
.claude/observability/.hm-lens-results/<slug>/<run-id>/<pass-id>/core.json
...
```bash
Bash("cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm lens_coverage check --results-dir .claude/observability/.hm-lens-results --slug <slug> --round <pass-id> --run-id <run-id> --preset Side")
```
...
**Four of the five arms below are stage-terminal; close the run on each of those**, with
`hm review_run close --slug <slug> --run-id <run-id> --outcome <APPROVED|CHANGES_REQUESTED>`.
**The `confirm-1` arm is NOT terminal and must NOT close** — it enters a repair round and
dispatches confirm-2, so closing there releases the slug *mid-pass* while `<run-id>` is still the
join key for the lens-results tree, the ledger rows below and the freeze refs. That is the arm
where releasing does the most damage, because reaching it means the review is still dirty.

> **The Grade Gate's APPROVE-side exits are NOT terminal and must NOT close.** This pass runs
> only when the gate would approve, so those exits flow into C1–C3 — closing there releases
> the slug while the freeze refs and ledger rows still use the id, and lets the next
> invocation mint a fresh id mid-review. (`close` is idempotent, so a second close is a no-op —
> not one of the harms.)
...
```bash
!cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm freeze reap --slug <slug>
```
...
```
Bash("cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm stage_agent_ledger emit --run-id '<run-id>' --agent confirmation-pass --stage review --slug '{slug}' --pass <1|2> --verdict '<PASS|FAIL>' --duration-ms '<elapsed>'")
```
...
```bash
Bash("cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm review_churn oscillation --slug {slug} --rounds <rounds>")
```
...
```bash
Bash("uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm review_telemetry emit --file <the literal temp path> --measured <FINALIZE-r{N}> <plus --measured <CHURN-r{N}> from round 2>")
```
...
**Leave `disposition_counts` and every `churn_*` key OUT of the row you write.** `--measured`
reads them from the payloads you `tee`'d at Step 4d and Step 5b.

> **`<FINALIZE-r{N}>` and `<CHURN-r{N}>` must be ABSOLUTE paths** — take them from `mktemp`, never
> a bare filename. The producers run from the task worktree and this command does
> not, so a relative path is written in one directory and looked for in another. That is the same
> producer/consumer split that made every measured field null before, arriving by a shorter
> route;
> `emit` reports the miss but the row still goes out empty. A value you supply is discarded
with a warning, and a payload whose stamped `slug`/`round` disagree with the row is refused, so a
stale file cannot become a wrong number. "No producer output" means you did not pass
`--measured` — fix that, never type the numbers in.
...
```
Bash("if [ -f \"<WT>/.claude/.hm-iter-receipts/.current-iter\" ]; then ITER=$(cat \"<WT>/.claude/.hm-iter-receipts/.current-iter\" 2>/dev/null); if [ -n \"$ITER\" ]; then uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm iter_receipts write --iter \"$ITER\" --stage review --verdict <verdict> --root \"<WT>\"; fi; fi")
```

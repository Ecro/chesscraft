...
1. **Phase decomposition** — Does each phase have a verifiable exit criterion? Are phases ordered by dependency? Do phases overlap or duplicate work? For nontrivial multi-phase plans, every phase must declare `depends_on`, `parallel_group`, and `merge_hazards`; shared file ownership must be serialized or called out as a merge hazard.

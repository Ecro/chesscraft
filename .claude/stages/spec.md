...
provenance: official
content_hash: 4e86492e12b50a5906baab539390d3f1d1e281892119019ee3493199b62add95
---
# Stage: spec
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 hm worktree task-preflight <slug> "$(pwd)" --stage hm:spec --claude-session-id "$HM_SESSION_ID"
```
...
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 hm autopilot_caps gate-blocked --root . --stage spec --session-id "$HM_SESSION_ID"
...
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 hm autopilot_caps boundary --root . --current spec --session-id "$HM_SESSION_ID" --step-cap 20 --time-cap-min 300

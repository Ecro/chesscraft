...
description: Procedure /hm:review follows for the auto-fix loop's round-state contract
  ...
  is non-empty.
content_hash: b7d22f87fa4bf69a0d7b1520be0b63d016304d3e016be4a8fd5fdbb2a836e1d1
---
...
```bash
uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.1 hm codex_adapter stamp-ids < <the temp path you wrote>
```
...
```bash
cd <the task worktree> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.1 hm second_opinion_oracle --findings-file <path> --root .
```
...
```bash
cd <the task worktree> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.1 hm second_opinion_invoke --record-disposition --disposition-file <the literal temp path> --slug "<slug>" --stage review
```

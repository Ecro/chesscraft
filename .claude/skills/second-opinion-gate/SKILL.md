...
description: Procedure /hm:review follows for the auto-fix loop's round-state contract
  ...
  is non-empty.
content_hash: 14bc8a12bc50216b9ed941d4275f2a56e60981ab879a1a1463f2f19dbe02552d
---
...
```bash
uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.49.0 hm codex_adapter stamp-ids < <the temp path you wrote>
```
...
```bash
cd <the task worktree> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.49.0 hm second_opinion_oracle --findings-file <path> --root .
```
...
```bash
cd <the task worktree> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.49.0 hm second_opinion_invoke --record-disposition --disposition-file <the literal temp path> --slug "<slug>" --stage review
```

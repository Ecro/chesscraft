...
description: Procedure /hm:review follows for the auto-fix loop's round-state contract
  ...
  is non-empty.
content_hash: 758ab39352639a451c0f7f7d41c9445c92039ef7a1a45564cf2911d0bad8e38e
---
...
```bash
uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.50.1 hm codex_adapter stamp-ids < <the temp path you wrote>
```
...
```bash
cd <the task worktree> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.50.1 hm second_opinion_oracle --findings-file <path> --root .
```
...
`severity` and `message`, and antigravity has no CLI-level schema at all), and the shipped
...
```bash
cd <the task worktree> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.50.1 hm second_opinion_invoke --record-disposition --disposition-file <the literal temp path> --slug "<slug>" --stage review
```

...
content_hash: 95f5aef2509882bf495007ea25c39a52a16dd27be7376a154b57d6c1d1796d6b
---
...
2. **If `worktree.enabled` is true → call the worktree CLI.**
   ...
   ```
   uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.5 python -m harness_maker.worktree create execute "$(pwd)"
   ```
...
4. **Finalize through the CLI, not direct merge/cleanup calls.**
   ...
   user-facing commit:
   ```bash
   uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.5 python -m harness_maker.worktree finalize <WT> stage-only
   ```

   Failure preserves the worktree for inspection:
   ```bash
   uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.5 python -m harness_maker.worktree finalize <WT> fail
   ```
   ...
   ```bash
   HM_OWNED_SESSION_UUIDS="$(uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.5 python -m harness_maker.worktree owned-crumb-read "$(pwd)" <slug>)" uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.5 python -m harness_maker.worktree post-commit-pop "$(pwd)"
   ```

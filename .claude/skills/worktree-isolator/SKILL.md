...
content_hash: eef3da29d0b121fb1b0d0faffdd052b93dec6a7d06c03fc61d2105f6e1e374dc
---
...
2. **If `worktree.enabled` is true → call the worktree CLI.**
   ...
   ```
   uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.50.1 python -m harness_maker.worktree create execute "$(pwd)"
   ```
...
4. **Finalize through the CLI, not direct merge/cleanup calls.**
   ...
   user-facing commit:
   ```bash
   uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.50.1 python -m harness_maker.worktree finalize <WT> stage-only
   ```

   Failure preserves the worktree for inspection:
   ```bash
   uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.50.1 python -m harness_maker.worktree finalize <WT> fail
   ```
   ...
   ```bash
   HM_OWNED_SESSION_UUIDS="$(uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.50.1 python -m harness_maker.worktree owned-crumb-read "$(pwd)" <slug>)" uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.50.1 python -m harness_maker.worktree post-commit-pop "$(pwd)"
   ```

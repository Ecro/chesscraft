...
content_hash: 96e7aacc0caf43994abaf4f209036651a96a6673215ec59faac3d335bd81ca63
---
# /hm:loop
...
/hm:loop --spec <path> [--mode feature|improve] [--target <path>]
         [--time 8h] [--max-iter 50] [--per-iter-stages execute,review]
...
- `--per-iter-stages <a,b,...>` — the atomic stages each iter runs, in order.
  Default: `execute,review` (wrapup is deferred to loop close to avoid a
  commit per iter). Recommended override: `plan,execute,review` for big
  features needing per-iter planning. Every name must be an atomic stage with a
  rendered `.claude/commands/hm/<stage>.md`; **`wrapup` is rejected** — loop close
  owns it, and running it per iter would commit and merge on every iteration.
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm worktree create execute "$(pwd)" --claude-session-id "$HM_SESSION_ID"
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm worktree verify <WT>
```
...
```bash
!if [ -z "$HM_SESSION_ID" ] || [ "<WT>" = "$(pwd)" ]; then touch .hm-loop-active; if [ -n "$CLAUDECODE" ] && [ -z "$HM_SESSION_ID" ]; then echo '⚠️ [loop] DEGRADED (HM_SESSION_ID unset in this shell — SessionStart env-file hook did not fire): this loop will STOP after one iteration. The Stop-hook has your session_id from stdin but the marker header is empty, so content-match fails and the loop is allowed to stop. Peers are NOT affected (a valid-id session ignores the global marker). Remedy: run /hm:health to confirm, fix the SessionStart env-file plumbing, or drive iterations manually.'; else echo '[loop] degraded marker (no isolation or non-Claude IDE) — parallel loops are NOT isolated'; fi; fi
```
...
| "operator decision to pause" / "user wants fresh context" | The loop body **cannot** use `AskUserQuestion` (autoloop autonomous-decision protocol). Any "operator decision" emitted from inside the loop body is a hallucination. |
...
3.5. **Write `.current-iter` marker** (Gate 0 contract — ADR-001 of PLAN-loop-mid-stop-and-review-skip).
   ...
   ```bash
   !uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm iter_receipts set-iter-marker --iter <N> --root "<WT>"
   ```
...
4.5. **Gate 0 — Receipt verification** (ADR-001 + ADR-005 of PLAN-loop-mid-stop-and-review-skip).
   ...
   ```bash
   !cd <WT> && uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm iter_receipts verify \
   ...
   **Auto-retry semantics (ADR-005, cap=2 per (iter, stage)):**
   ...
   2. Read the current value of `stage_retry_counts[K]` from
      ...
      ```bash
      !uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm iter_receipts patch-runtime \
   ...
   4. If `stage_retry_counts[K] >= 3`:
      Escalate via `AskQuestion` (Cursor) or `AskUserQuestion` (Claude Code).
      Options:
...
5. **Finalize worktree** (only if engaged in step 5):
   ...
   ```bash
   !uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm worktree finalize <WT> <STATUS>
   ```

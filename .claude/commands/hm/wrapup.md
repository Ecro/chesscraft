...
description: 'Close the unit of work: final verification, memory capture, and the
  single commit.'
content_hash: 82502bab76207741485bf3ed0a94d7f728d9d71f4597255843d1271253e49a07
---
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.1 hm worktree task-preflight <slug> "$(pwd)" --stage hm:wrapup --claude-session-id "$HM_SESSION_ID"
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.1 hm spec_machine mark-tested \
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.1 hm spec_machine mark-judged \
   --yaml specs/SPEC-{slug}.machine.yaml \
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.1 hm spec_machine waiver-check --yaml specs/SPEC-{slug}.machine.yaml --dev-mode task-driven --root .
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.1 hm memory_md upsert-wiki --root . --slug '<slug>' --category '<category>' --body-file <tmpfile>
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.1 hm memory_retrieve --topic "<symptom / root cause>" --k 6 --pre-k 30
```
...
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.1 hm memory_md upsert-failure --root . --slug '<slug>' --category '<category>' --body-file <tmpfile>
# Recurrence (confident same-root-cause match) — reuse the EXACT slug + one-line note:
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.1 hm memory_md upsert-failure --root . --slug '<existing-slug>' --category '<category>' --occurrence-note '<one line: what happened this time>'
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.1 hm second_brain promote --type <decision|failure|preference|project|reference|journal> --source-slug '<stable-local-slug>' --title '<title>' --body-file <path>
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.1 hm worktree task-land <SLUG> <BASE>
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.1 hm worktree commit-base-memory <BASE> --expect-head <SQUASH_SHA>
```
...
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.1 hm autopilot_caps gate-blocked --root . --stage wrapup --session-id "$HM_SESSION_ID"
...
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.1 hm autopilot_caps boundary --root . --current wrapup --session-id "$HM_SESSION_ID" --step-cap 20 --time-cap-min 300

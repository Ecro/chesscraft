...
description: Delivery-metrics trend — change-failure rate and post-merge churn, with
  interpretation.
content_hash: 892096458fecc0fa1029bf253509fa7985926f3684211228a6efab3e66d7d541
---
# /hm:metrics
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.4 hm delivery_metrics candidates --root .
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.4 hm delivery_metrics adjudicate --root . --commit <sha> --release <ref> --verdict <remediation|routine> --reason "<one line>"
```

## Step 3 — Compute the snapshot

```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.4 hm delivery_metrics compute --root .
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.4 hm delivery_metrics trend --root . --limit 12
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.4 hm run_classify boundaries --root .
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.4 hm run_classify record --root . --boundary-uuid <uuid> --verdict <continuation|new|unknown> --reason "<one line>"
```

### 5b — Compute the mix

```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.4 hm economics report --root .
```

...
description: Delivery-metrics trend — change-failure rate and post-merge churn, with
  interpretation.
content_hash: ecc937f851b9169c5881fece492c47ca0aa329a4566ff1870527af62e1e9abc7
---
# /hm:metrics
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm delivery_metrics candidates --root .
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm delivery_metrics adjudicate --root . --commit <sha> --release <ref> --verdict <remediation|routine> --reason "<one line>"
```

## Step 3 — Compute the snapshot

```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm delivery_metrics compute --root .
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm delivery_metrics trend --root . --limit 12
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm run_classify boundaries --root .
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm run_classify record --root . --boundary-uuid <uuid> --verdict <continuation|new|unknown> --reason "<one line>"
```

### 5b — Compute the mix

```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm economics report --root .
```

...
description: Delivery-metrics trend — change-failure rate and post-merge churn, with
  interpretation.
content_hash: a3cbe9af9c9a8f6c829d8feb6e2fb66577bf320ea6e917fa0da6e0c67a83468d
---
# /hm:metrics
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.55.0 hm delivery_metrics candidates --root .
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.55.0 hm delivery_metrics adjudicate --root . --commit <sha> --release <ref> --verdict <remediation|routine> --reason "<one line>"
```

## Step 3 — Compute the snapshot

```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.55.0 hm delivery_metrics compute --root .
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.55.0 hm delivery_metrics trend --root . --limit 12
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.55.0 hm run_classify boundaries --root .
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.55.0 hm run_classify record --root . --boundary-uuid <uuid> --verdict <continuation|new|unknown> --reason "<one line>"
```

### 5b — Compute the mix

```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.55.0 hm economics report --root .
```

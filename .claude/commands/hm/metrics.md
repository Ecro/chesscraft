...
description: Delivery-metrics trend — change-failure rate and post-merge churn, with
  interpretation.
content_hash: 79f81046789c936541025c8bc43c0fb6e3dea0a269fd7a4243310ffc2ac8a97d
---
# /hm:metrics
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.50.1 hm delivery_metrics candidates --root .
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.50.1 hm delivery_metrics adjudicate --root . --commit <sha> --release <ref> --verdict <remediation|routine> --reason "<one line>"
```

## Step 3 — Compute the snapshot

```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.50.1 hm delivery_metrics compute --root .
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.50.1 hm delivery_metrics trend --root . --limit 12
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.50.1 hm run_classify boundaries --root .
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.50.1 hm run_classify record --root . --boundary-uuid <uuid> --verdict <continuation|new|unknown> --reason "<one line>"
```

### 5b — Compute the mix

```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.50.1 hm economics report --root .
```

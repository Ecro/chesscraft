...
description: Delivery-metrics trend — change-failure rate and post-merge churn, with
  interpretation.
content_hash: f7ee8fc2c7e713f4e59b68052fa99ee58e0fd3e871a1972cc18b44a41c033438
---
# /hm:metrics
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.51.1 hm delivery_metrics candidates --root .
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.51.1 hm delivery_metrics adjudicate --root . --commit <sha> --release <ref> --verdict <remediation|routine> --reason "<one line>"
```

## Step 3 — Compute the snapshot

```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.51.1 hm delivery_metrics compute --root .
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.51.1 hm delivery_metrics trend --root . --limit 12
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.51.1 hm run_classify boundaries --root .
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.51.1 hm run_classify record --root . --boundary-uuid <uuid> --verdict <continuation|new|unknown> --reason "<one line>"
```

### 5b — Compute the mix

```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.51.1 hm economics report --root .
```

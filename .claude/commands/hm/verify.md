...
description: Pre-completion stop sign — deterministic regression, structure and security
  checks.
content_hash: a90c26eea6cdae441bd20544005b756fa295cd103556609bac71301adc73cade
---
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 hm worktree task-preflight <slug> "$(pwd)" --stage hm:verify --claude-session-id "$HM_SESSION_ID"
```
...
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 hm autopilot_caps gate-blocked --root . --stage verify --session-id "$HM_SESSION_ID"
...
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.54.0 hm autopilot_caps boundary --root . --current verify --session-id "$HM_SESSION_ID" --step-cap 20 --time-cap-min 300

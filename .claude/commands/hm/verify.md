...
description: Pre-completion stop sign — deterministic regression, structure and security
  checks.
content_hash: 039db851f90502dd2d55d79b15caed250a7c87a4dc12b6be6a0f853424efe77a
---
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.5 hm worktree task-preflight <slug> "$(pwd)" --stage hm:verify --claude-session-id "$HM_SESSION_ID"
```
...
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.5 hm autopilot_caps gate-blocked --root . --stage verify --session-id "$HM_SESSION_ID"
...
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.5 hm autopilot_caps boundary --root . --current verify --session-id "$HM_SESSION_ID" --step-cap 20 --time-cap-min 300

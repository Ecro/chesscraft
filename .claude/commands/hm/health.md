...
content_hash: a5fa3049b773eb731d729290cc6c7099c4606bb6ee2e8cbee60a0412a8e555ae
---
# /hm:health
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.55.0 hm cli health . --session-id "$HM_SESSION_ID" --json-output .claude/observability/.health.tmp.json
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.55.0 hm worktree drain .
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.55.0 hm second_opinion_invoke --model codex --smoke --slug health-smoke --stage health
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.55.0 hm autopilot_ledger smoke --root . --level auto_safe
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.55.0 hm economics doctor --root .
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.55.0 hm delivery_metrics trend --root . --limit 1
```

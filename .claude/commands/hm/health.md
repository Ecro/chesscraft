...
content_hash: 6849ef4ae23752153cc6d944097bb79e4745b59ae9bf00e9c3f07237a6df624d
---
# /hm:health
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.51.1 hm cli health . --session-id "$HM_SESSION_ID" --json-output .claude/observability/.health.tmp.json
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.51.1 hm worktree drain .
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.51.1 hm second_opinion_invoke --model codex --smoke --slug health-smoke --stage health
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.51.1 hm autopilot_ledger smoke --root . --level auto_safe
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.51.1 hm economics doctor --root .
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.51.1 hm delivery_metrics trend --root . --limit 1
```

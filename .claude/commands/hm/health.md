...
content_hash: 57f2ec9f1f04e70543991747ed9816c99e19036acd50a6f355306107f0ec17ff
---
# /hm:health
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.6 hm cli health . --session-id "$HM_SESSION_ID" --json-output .claude/observability/.health.tmp.json
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.6 hm worktree drain .
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.6 hm second_opinion_invoke --model codex --smoke --slug health-smoke --stage health
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.6 hm autopilot_ledger smoke --root . --level auto_safe
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.6 hm economics doctor --root .
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.52.6 hm delivery_metrics trend --root . --limit 1
```

...
content_hash: beb17c2bdd712b4087ca5aa976d292847618044f878484bc946a0c17dc07175f
---
# /hm:health
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm cli health . --session-id "$HM_SESSION_ID" --json-output .claude/observability/.health.tmp.json
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm worktree drain .
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm second_opinion_invoke --model codex --smoke --slug health-smoke --stage health
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm autopilot_ledger smoke --root . --level auto_safe --targets claude-code,codex
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm economics doctor --root .
```
...
```bash
!uv run --with $HOME/.claude/plugins/cache/harness-maker/harness-maker/0.57.1 hm delivery_metrics trend --root . --limit 1
```

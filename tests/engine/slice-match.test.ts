...
describe('a match played with slice content only', () => {
  ...
  it('does not stall on the sixth turn when the pool cannot fill a second offer', () => {
    // The slice preset ships the bare minimum of 3 skill cards. Both are dealt
    // in the first offer, so the AC-006 second draft has nothing disjoint left
    // to offer. The absent case must be "no second offer", not "an empty offer"
    // — an empty offer still gates board play and freezes the match.
    const content = loadSliceContent()

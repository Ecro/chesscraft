...
describe('initialSource — a saved install with no stamp (ADR-003, the absent case)', () => {
  // Every install that exists today. The stamp is synthesised from the current
  // bundle, so nothing is new relative to it: the catalogue the author had is
  // exactly the catalogue they get, and the backlog is forgone once.
  ...
  it('changes nothing', () => {
    ...
    expect(result.mergeFailed).toEqual([])
    expect(ids(result.source.presets)).not.toContain('preset.covenant')
